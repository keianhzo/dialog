...
const MEDIA_FILE = "/Users/mozilla/Downloads/sample-mov-file.mov"
...
module.exports = class GStreamer {
  ...
  get _commandArgs () {
    let commandArgs = [
      ...
      'opusenc',
      '!',
      `rtpopuspay pt=${this._outParams.payload_type} ssrc=${this._outParams.ssrc}`,
      '!',
      'rtpbin.send_rtp_sink_0',
      `rtpbin.send_rtp_src_0 ! udpsink bind-address=127.0.0.1 host=${this._outParams.audioTransportIp} port=${this._outParams.audioTransportPort} bind-port=${this._outParams.localRtpPort}`,
      `rtpbin.send_rtcp_src_0 ! udpsink bind-address=127.0.0.1 host=${this._outParams.audioTransportIp} port=${this._outParams.audioTransportRtcpPort} sync=false async=false udpsrc port=${this._outParams.localRtcpPort}`
    ];

    // let commandArgs = [
    //   `rtpbin name=rtpbin latency=50 buffer-mode=0 sdes="application/x-rtp-source-sdes, cname=(string)${this._inParams.rtpParameters.rtcp.cname}"`,
    //   '!'
    // ];
